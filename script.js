  ...
  const CL_STORAGE_KEY = 'eru_bt_checklist_v3';
  ...
  function renderClItem(item, listWrap, state) {
    if (item.type === 'course') {
  ...
  function renderClList() {
    ...
    const orderedSemesters = [
      ...Object.entries(clData.common.semesters),
      ...Object.entries(track.semesters),
    ];

    orderedSemesters.forEach(([semName, items]) => {
      const block = document.createElement('div');
      block.className = 'cl-semester-block';

      const totalCr = items.reduce((s, it) => s + it.cr, 0);
      const titleRow = document.createElement('div');
      titleRow.className = 'cl-semester-title';
      titleRow.innerHTML = `
        <span class="cl-semester-badge">${semName}</span>
        <span class="cl-semester-line"></span>
        <span class="cl-semester-credits">${totalCr} ساعة</span>
      `;
      block.appendChild(titleRow);

      items.forEach(item => renderClItem(item, block, state));
  ...
  function updateClProgress() {
    ...
    const track = clData[clCurrentTrack];
    const allItems = [
      ...Object.values(clData.common.semesters).flat(),
      ...Object.values(track.semesters).flat(),
    ];
    let total = 0, done = 0;
